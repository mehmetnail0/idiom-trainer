        ...
        <div className="space-y-4 animate-[fadeIn_0.25s_ease-out]">
          ...
          <div className="rounded-lg p-4 space-y-2.5 border border-border/30 bg-bg/50">
            <div className="flex justify-between items-baseline">
              ...
              <a href={url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-text-dim hover:text-accent transition-colors">cambridge →</a>
            </div>
            ...
            <div className="space-y-1 pt-1">
              {item.examples.map((ex, j) => (
                <p key={j} className="text-[11px] text-text-dim/70 italic pl-2.5 border-l border-border/30">{ex}</p>
              ))}
            </div>
            {item.wrongExample && (
              <p className="text-[11px] text-wrong/50 pl-2.5 border-l border-wrong/20 italic">✗ {item.wrongExample}</p>
            )}
          </div>

          {saved ? (
            <p className="text-[10px] text-correct/60 text-center">saved ✓</p>
          ) : correct ? (
