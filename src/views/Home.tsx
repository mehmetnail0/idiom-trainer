...
export default function Home({ onStart }: { onStart: () => void }) {
  ...
  const hot = activeItems.filter(i => getHeat(i) >= 0.75).length
  const warm = activeItems.filter(i => getHeat(i) > 0 && getHeat(i) < 0.75).length
  const fresh = activeItems.filter(i => getHeat(i) === 0).length
  const dueNow = activeItems.filter(i => isDue(i)).length
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      <div>
        ...
        <p className="text-[11px] text-text-dim/50 mt-1">FSRS · {activeItems.length} items · {dueNow} due</p>
      </div>
      ...
      <div>
        ...
        <div className="h-1.5 bg-border/40 rounded-full overflow-hidden flex">
          {hot > 0 && <div className="bg-correct/60 transition-all duration-700" style={{ width: `${(hot / activeItems.length) * 100}%` }} />}
          {warm > 0 && <div className="bg-accent/50 transition-all duration-700" style={{ width: `${(warm / activeItems.length) * 100}%` }} />}
        </div>
      </div>
      ...
      <div className="flex justify-between text-[10px] text-text-dim/25">
        <span>{items.filter(i => i.type === 'idiom').length} idioms · {items.filter(i => i.type === 'word').length} words{items.filter(i => i.archived).length > 0 ? ` · ${items.filter(i => i.archived).length} archived` : ''}</span>
